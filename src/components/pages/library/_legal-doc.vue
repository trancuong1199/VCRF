<template>
  ...
  <div class="container my-16">
      ...
      <transition name="slide-translate" mode="out-in">
        <table class="text-[10px] tablet:text-base">
          ...
          <tr v-for="doc in docsData" :key="doc.id">
            ...
            <td>{{ doc.create_at }}</td>
            <td>
              <img class="cursor-pointer w-[12px] tablet:w-[24px] h-[12px] tablet:h-[24px]" v-lazy="downLoadFileIc"
                alt="download file" @click="() => handleDownload(doc.uploaded_file, doc.name)" />
            </td>
          </tr>
        </table>
      </transition>
    ...
    <div class="btn view-more flex justify-center">
      <button @click="loadMore" class="bg-darkYellow w-[180px] h[40px] rounded-full font-bold">{{ t('view-more')
      }}</button>
    </div>
  </div>
</template>
